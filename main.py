...
mystr = "个萄七葡木业语红是汁那哥咖明十他期人五几月和日物食要氵灬果里老料忄国在我色爷包冰水只大住米她饱父犬的奶喝六妳花岁想妈男惊吃酒这忙饭狗今家去茶休们姐门二九田心乐谁雨牛饣啡婆师你讠中禾好公面白猫冷哪爸小也作外天妹手四言目星可母三文不八儿土热弟一纟汉力饮吗糸火啤扌"
...
printmd("This app has dynamic probabilities per char and that are updated following your score.")
printmd("P❤️A")
printmd('----')
...
def update_w(weights,indx,isCorrect):
    mydelta = 0.05*(1/len(weights))
    if isCorrect:
...
def askarandom(mydict=mydict):
    ...
    while please_continue:
        ...
        #
        weights = update_w(weights,rand_index,ans in true_ans)
        #
        if ans in true_ans:
            print('** correct **')
            ...
            printmd('** wrong, answer is > '+str(true_ans)+' **')
